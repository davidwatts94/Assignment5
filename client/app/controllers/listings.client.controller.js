angular.module('listings').controller('ListingsController', ['$scope', '$location', '$stateParams', '$state', 'Listings', 
  function($scope, $location, $stateParams, $state, Listings){
    ...
    $scope.update = function(isValid) {
      ...
      var updateID = $stateParams.listingId;
      ...
      // Create the listing object: code copied from above
      var listing = {
        name: $scope.name, 
        code: $scope.code, 
        address: $scope.address
      ...
      //slightly modifed version of Listings.create
      Listings.update(updateID, listing)
        .then(function(response) {
          //if the object is successfully saved redirect back to the list page
    ...
    $scope.remove = function() {
      ...
      var removeThis = $stateParams.listingId; //remove this ID
      
      //modified version of Listings.update
      Listings.delete(removeThis)
        .then(function(response) {
          //if the object is successfully saved redirect back to the list page
